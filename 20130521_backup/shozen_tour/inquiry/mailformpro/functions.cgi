## 2010-10-09 mailform pro Ver.2.1.4 functions file
...
sub getpost {
	...
	foreach $pair (@pairs) {
		...
		$value =~ s/\\n/\n/g;
		if($name ne $null && $name ne "Submit" && $name ne "confirm_email" && $name ne "x" && $name ne "y" && $name ne "must_id" && $name ne "input_time" && $name ne "javascript_flag" && $name ne "http_referer" && $name ne "mailform_confirm_mode" && index($name,'[unjoin]') == -1 && $name ne "sitein_referrer" && $name ne "confirm_time"){
			if($name ne $prevName){
				$crr = "";
				if(index($value,"\n") > -1){
					$crr = "\n";
...
sub refresh {
	...
	if(index($refreshurl,'http') == -1){
		$uri = (split(/send\.cgi/,$ENV{'SCRIPT_NAME'}))[0];
